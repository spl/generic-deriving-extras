...
data Step :: * -> * -> * where
  Root :: Step a a -> Step a a
  Down :: Dir -> String -> proxy a -> Step a b -> Step r b
  Move :: Dir -> String -> proxy a -> Step a b -> Step r b
  End  :: Step a b
...
runZipperT z = liftM changeResults (runErrorT (runStateT (unZipperT z) noMsg))
  where
